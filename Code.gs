...
function doPost(e) {
  const p = e && e.parameter ? e.parameter : {};
  try {
    const req = (e.postData && String(e.postData.type || '').indexOf('application/json') >= 0)
...
function route_(r) {
  ...
  if (['getDashboard','updateCheck','editData','getHistory','startNewShift','getUsers'].indexOf(r.action) >= 0) return routeAuthenticated_(r);
...
function loginPage_(origin) {
  ...
  let message;
  try {
    const googleUser = currentGoogleUser_();
    const user = findUser_(googleUser.email);
    if (!user) throw new Error('Email ' + googleUser.email + ' belum terdaftar di Users.');
    if (String(user.status).toUpperCase() !== 'ACTIVE') throw new Error('Akun sedang tidak aktif.');
    const token = Utilities.getUuid() + Utilities.getUuid().replace(/-/g,'');
    CacheService.getScriptCache().put('SESSION_' + token, JSON.stringify({email:googleUser.email,created:Date.now()}), APP.TOKEN_TTL);
    message = {type:'OFFICE_DASHBOARD_LOGIN',token:token,email:googleUser.email};
  } catch (err) {
...
function currentGoogleUser_(){
  ...
  if(!email) throw new Error('Google tidak memberikan email. Deploy Web App sebagai "User accessing the web app" dan batasi akses ke akun Google yang berwenang.');
  return {email};
}
function authenticatedUser_(token){
  ...
  const session=JSON.parse(raw);const user=findUser_(session.email);
  if(!user||String(user.status).toUpperCase()!=='ACTIVE') throw new Error('Akun tidak aktif atau sudah dihapus.');
  return user;
}
function findUser_(email){
  const s=SpreadsheetApp.getActive().getSheetByName(APP.USER_SHEET);if(!s||s.getLastRow()<2)return null;
  email=String(email).trim().toLowerCase();const rows=s.getRange(2,1,s.getLastRow()-1,4).getValues();
  for(const r of rows)if(String(r[0]).trim().toLowerCase()===email)return{email:r[0],name:r[1],role:r[2],status:r[3]};
  return null;
}

function setupDashboard(){
  const ss=SpreadsheetApp.getActive();if(!ss.getSheetByName(APP.DATA_SHEET))throw new Error('Sheet DATA BANK tidak ditemukan.');
...
function ensureSheet_(name,headers){const ss=SpreadsheetApp.getActive();let s=ss.getSheetByName(name);if(!s)s=ss.insertSheet(name);if(s.getLastRow()===0)s.appendRow(headers);return s}
...
function getDashboardForUser_(user){
  const s=SpreadsheetApp.getActive().getSheetByName(APP.DATA_SHEET);if(!s)throw new Error('DATA BANK tidak ditemukan.');
  const data={
...
function readCategory_(sheet,nameCol,checkCol,bankCol,startRow){
  const last=sheet.getLastRow();if(last<startRow)return[];const max=Math.max(nameCol,checkCol,bankCol);const rows=sheet.getRange(startRow,1,last-startRow+1,max).getValues();const out=[];
  rows.forEach((r,i)=>{const row=startRow+i,name=r[nameCol-1],bank=r[bankCol-1],checked=r[checkCol-1]===true;if(name===''&&bank==='')return;out.push({row,number:row-startRow+1,bank,name,checked,nameCell:col_(nameCol)+row,checkCell:col_(checkCol)+row})});return out;
}
function getShift_(){const s=SpreadsheetApp.getActive().getSheetByName(APP.SHIFT_SHEET);if(!s)throw new Error('Shift_Control belum dibuat. Jalankan setupDashboard().');return{shift:s.getRange('B2').getDisplayValue(),startTime:s.getRange('B3').getDisplayValue(),startedBy:s.getRange('B4').getDisplayValue()}}
