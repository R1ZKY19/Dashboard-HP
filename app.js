...
function api(action,data={}){
  return new Promise((resolve,reject)=>{
    ...
    iframe.name="api_"+id;iframe.style.display="none";document.body.appendChild(iframe);
...
function bindEvents(){
  ...
  if(ep)ep.addEventListener("keydown",e=>{if(e.key==="Enter")startLogin()});
  const ee=document.getElementById("loginEmail");
  if(ee)ee.addEventListener("keydown",e=>{if(e.key==="Enter")startLogin()});
  const logout=document.getElementById("logoutBtn");if(logout)logout.onclick=logoutUser;
...
async function startLogin(){
  ...
  try{
    showLoading();setLoginStatus("Memeriksa akun...");
    const result=await api("login",{email,password});
    if(!result?.token)throw new Error("Login gagal: token tidak diterima.");
    sessionToken=result.token;localStorage.setItem(CONFIG.SESSION_KEY,sessionToken);
    dashboard=await api("getDashboard");renderUser();renderStats();renderTable();setLoginStatus("");
...
function renderTable(){
  const q=document.getElementById("searchInput").value.toLowerCase().trim(),filter=document.getElementById("checkFilter").value;
