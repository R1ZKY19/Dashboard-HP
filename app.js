...
async function api(action,data={}){
  if(!CONFIG.API_URL) throw new Error("API_URL belum diisi di config.js");
  const r=await fetch(CONFIG.API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify({action,token:sessionToken,...data})});
...
async function init(){
  ...
  try{showLoading();dashboard=await api("getDashboard");renderUser();renderStats();renderTable();}
  catch(e){clearSession();showLogin();setLoginStatus(e.message);}
  finally{hideLoading();}
}
...
function startLogin(){
  ...
  if(!loginPopup){setLoginStatus("Popup login diblokir browser. Izinkan popup untuk situs ini lalu klik MASUK lagi.");return;}
  const timer=setInterval(()=>{if(loginPopup.closed){clearInterval(timer);}},500);
}

window.addEventListener("message",async event=>{
  if(event.origin!==new URL(CONFIG.API_URL).origin)return;
  const data=event.data||{};
  if(data.type!=="OFFICE_DASHBOARD_LOGIN")return;
  if(!data.token){setLoginStatus(data.message||"Login gagal.");return;}
  sessionToken=data.token;
  localStorage.setItem(CONFIG.SESSION_KEY,sessionToken);
  try{showLoading();dashboard=await api("getDashboard");renderUser();renderStats();renderTable();setLoginStatus("");}
  catch(e){clearSession();showLogin();setLoginStatus(e.message);}
  finally{hideLoading();}
});
...
function renderTable(){
  const q=document.getElementById("searchInput").value.toLowerCase().trim();const filter=document.getElementById("checkFilter").value;
